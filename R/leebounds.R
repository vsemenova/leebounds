...
basic_lee_bound<-function(leedata,treat_helps=NULL,...) {
  # print(treat_helps)
  d<-leedata$treat
  s<-leedata$selection
  sy<-leedata$outcome
...
discrete_bound<-function(leedata, treat_helps=TRUE) {
  ...
  p0s_by_group<-matrix(unlist(lapply(group_map(leedata,basic_lee_bound,treat_helps=treat_helps, .keep=TRUE),GetThresh)),ncol=1,byrow=TRUE)
   
  ## calculate group-specific bounds and p0s
  bounds_by_group<-matrix(unlist(lapply(group_map(leedata,basic_lee_bound,treat_helps=treat_helps, .keep=TRUE),GetBounds)),ncol=2,byrow=TRUE)
  
  
    if (treat_helps==TRUE) {
      inds<-leedata$selection==1 & leedata$treat ==0 
      
      pmf<-wtd.table(leedata$group[inds],weights=leedata$weights[inds])/sum(wtd.table(leedata$group[inds],weights=leedata$weights[inds]))
