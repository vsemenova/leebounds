...
numeric_names<-c("AGE","AGEARCAT","AGEPARNT","EARN_CMP","EARN_YR",
...
#write_feather(Lee_data_covariates[,selected_names],paste0(my_path,"/DERIVED_DATA/dataLee2009covariates_short.feather"))
...
#
#covariate_data<-covariate_data[,setdiff(colnames(covariate_data),"MRPID")]

#almost_zero=apply(as.matrix(Lee_data_covariates[,my_names]),2,function(x) mean(x))

## drop covariates with zero variation
#Lee_data_covariates<-Lee_data_covariates[, !(almost_zero<0.001)]


write_feather(Lee_data_covariates,paste0(my_path,"/DERIVED_DATA/dataLee2009covariates_short.feather"))
